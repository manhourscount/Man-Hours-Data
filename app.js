...
function calculateMetrics() {
 ...
 // Always read the total directly out of the editable panel element
...
// Loads historical calculations or sets values onto the editable input view
...
document.addEventListener('DOMContentLoaded', () => {
 ...
 // Keep input editable across all filter profiles
 absentInput.disabled = false;
 loadAbsenteeInputData();

 // Write-back redirection processor
 absentInput.addEventListener('input', () => {
     ...
     if (currentMode === 'LIVE') {
         ...
         // Determine which archive month is being actively adjusted
         let targetMonthIdx = localNow.getMonth();
         if (currentMode.startsWith('M-')) {
             targetMonthIdx = parseInt(currentMode.split('-')[1]);
         }
         
         // Clear old logs out for this month and overwrite with the new flat input entry on Day 1
         ...
         // Store the entire monthly quantity directly onto day 1 of that active profile block
         monthlyAbsenteeStorage[formatDateKey(startOfMonth)] = rawValue;
     } else if (currentMode === 'YTD') {
         // Clear the whole calendar database and place the entire aggregate entry on Jan 1
