// ==========================================
// 1. GLOBAL INITIALIZATION & FIREBASE SETUP
// ==========================================
...
// Target Live Database Configuration
...
// Initialize Firebase Core and DB Nodes
const app = initializeApp(firebaseConfig);
const db = getDatabase(app);
const dbRef = ref(db, 'arcadia_absentee_db');

// Multi-client shared dynamic memory state
let monthlyAbsenteeStorage = {};
let previousTimeFilterMode = 'LIVE';

// Realtime Stream Interceptor for Cross-PC Updates
onValue(dbRef, (snapshot) => {
 const cloudData = snapshot.val();
 monthlyAbsenteeStorage = cloudData || {};
 
 // Force UI to instantly react when other clients edit values
...
function getWorkingDaysCount(mode) {
 ...
 const targetMonth = parseInt(mode.split('-')[1]); // ✨ Fixed
...
function exportMetrics() {
 ...
 const monthIdx = parseInt(timeFilter.value.split('-')[1]); // ✨ Fixed
...
// ==========================================
// 5. DATA SYNC & ATTACHED EVENT HOOKS
// ==========================================
...
function saveAbsenteeProgressToDisk() {
 set(dbRef, monthlyAbsenteeStorage)
